<div class="row-{row.id}-bg text-center {!isEnabled ? 'hidden' : ''}" style={rowBody} bind:clientWidth={width} bind:this={thisDiv}>
    {#if isEnabled}
...
<script lang="ts">
    ...
    import { app, getStyling, checkRequirements, pointTypeMap, rowDesignMap, sanitizeArg, checkActivated, globalReqMap, replaceText, choiceMap, activatedMap, variableMap } from '$lib/store/store.svelte';
    ...
    let rowTitle = $derived.by(() => {
        return `white-space: pre-wrap;font-family: ${textStyle.rowTitle}; font-size: ${textStyle.rowTitleTextSize}%; text-align: ${textStyle.rowTitleAlign}; color: ${textStyle.rowTitleColor}`;
    });
