...
export const appVersion = '2.0.0-beta-0.9.3';
...
export async function saveToSlot(slotData: SaveSlot, key: string, index: number, storeKey: string, isBuild?: boolean) {
    ...
    try {
        ...
        if (isBuild) {
            ...
            saveSlots[index].app = slotData.app;
        }
    } catch (error) {
        console.error(error);
    }
...
export async function loadFromSlot(key: string, storeKey: string): Promise<any | null> {
    const db = await getDB();
    return new Promise((resolve, reject) => {
            ...
            if (typeof key === 'string') {
                if (key.startsWith(regex)) {
                    ...
                    if (suffix === 'autoSave') {
                        ...
                        autoSaveSlot.app = value.app;
                    } else {
                        const match = suffix.match(/^slot-(\d+)$/);
                        const index = Number(match?.[1]);
                        saveSlots[index].stored = true;
                        saveSlots[index].name = value.name ?? `Slot ${index + 1}`;
                        saveSlots[index].time = value.time;
                        saveSlots[index].app = value.app;
                    }
                }
            }
